...
def test_successful_token_refresh(requests_mock, client):
    ...
    requests_mock.post(
        ...
        json={"access_token": "test_access_token", "id_token": "test_id_token"},
        status_code=200,
    )
    ...
    payload = {"code": "test_code", "redirect_uri": "test_redirect_uri"}

    login_response = client.post(LOGIN_ENDPOINT, data=payload, format="json")

    refresh_response = client.post(
        REFRESH_ENDPOINT, data={"refresh_token": login_response.data["refresh_token"]}, format="json"
    )
